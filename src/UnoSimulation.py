...
class UnoSimulation:
    ...
    def round(self):
        ...
        for i in range(1, self.number_of_players+1):
            print(">>>> Player ",i)
            for card in self.IA_UNO.players.get_player_by_index(i).me_player.my_cards:
                print(card[0],' - ',card[1])
        ...
        while(True):
            ...
            card_thrown = self.IA_UNO.players.get_player_by_index(self.IA_UNO.INDEX_WHO_IS_PLAYING).move()    
            self.card_on_the_table = card_thrown
            print(self.IA_UNO.players.get_player_by_index(self.IA_UNO.INDEX_WHO_IS_PLAYING).me_player.name," - ",card_thrown)
            if(len(card_thrown) == 2): #has an action card
                self.IA_UNO.applying_action_card(card_thrown)
            if(card_thrown[0] == 'G'):
                print(self.IA_UNO.players.get_player_by_index(self.IA_UNO.INDEX_WHO_IS_PLAYING).me_player.name)
                break
