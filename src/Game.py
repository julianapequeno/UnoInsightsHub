...
class Game:
    ...
    def verify_card_action_in_game(self,card_action): # ALGORITMO QUE VERIFICA ON IMPACTO DAS CARTAS JOGADAS NO JOGO
        ...
        if(card == 'X'): # BLOQUEIO
            ...
            for new_card in card_action[1]:
                self.active_players.get_player_by_index(self.index_who_is_playing+1).get_new_card(new_card)
    ...
    def rodada(self):
        ...
        while(True):
            ...
            list_card_action = self.start_player_turn(self.who_is_currently_playing)
